...
import com.imovel.api.payment.PaymentGatewayFactory;
...
public class PaymentService implements IPaymentService {
    ...
    @Override
    public ApplicationResponse<PaymentResponse> processPayment(Long userId, BigDecimal amount, String currency,String description) {
        try {

            if (amount.compareTo(BigDecimal.ZERO) <= 0) {
                return ApplicationResponse.success(null, "No payment needed");
            }

            PaymentRequest request = new PaymentRequest(description,amount, 1L, currency);

            PaymentGatewayFactory factory = new PaymentGatewayFactory();

            PaymentGateway paymentGateway = factory.createPaymentGateway(gatewayProviderType);
            PaymentResponse paymentResponse = paymentGateway.processPayment(request);
            
            if (paymentResponse != null
                    && paymentResponse.getStatus().equals("SUCCESS")) {
                return ApplicationResponse.success(paymentResponse, "Payment processed successfully");
    ...
    @Override
    public ApplicationResponse<PaymentResponse> processRefund(Long userId, BigDecimal amount, String currency,String description) {

        try {
            if (amount.compareTo(BigDecimal.ZERO) <= 0) {
                return ApplicationResponse.success(null, "No refund needed");
            }
            ...
            if (paymentResponse != null && paymentResponse.getStatus().equals("SUCCESS")) {
                return ApplicationResponse.success(paymentResponse, "Refund processed successfully");
            } else {
                return ApplicationResponse.error(
                        ApiCode.PAYMENT_GATEWAY_ERROR.getCode(),
